...
int ESP32can::begin(long baudRate)
{
...
int ESP32can::read(uint32_t &_rxId, uint8_t &_rxLength, uint8_t* _rxData, bool &_rxExtended) {
  ...
  bool _rxRtr = (readRegister(REG_SFF) & 0x40) ? true : false;
  int _rxDlc = (readRegister(REG_SFF) & 0x0f);
  int _rxIndex = 0;
...
void ESP32can::onReceive(void(*callback)(int))
{
  ...
  if (callback) {
    esp_intr_alloc(ETS_CAN_INTR_SOURCE, 0, onInterrupt, this, &_intrHandle);
  }
}

int ESP32can::filter(int id, int mask)
{
  id &= 0x7ff;
  mask = ~(mask & 0x7ff);
...
int ESP32can::filterExtended(long id, long mask)
{
  id &= 0x1FFFFFFF;
...
void ESP32can::setPins(int rx, int tx)
{
  _rxPin = (gpio_num_t)rx;
  _txPin = (gpio_num_t)tx;
}

void ESP32can::dumpRegisters(Stream& out)
{
  for (int i = 0; i < 32; i++) {
...
uint8_t ESP32can::readRegister(uint8_t address)
{
...
void ESP32can::modifyRegister(uint8_t address, uint8_t mask, uint8_t value)
{
  volatile uint32_t* reg = (volatile uint32_t*)(REG_BASE + address * 4);
...
void ESP32can::writeRegister(uint8_t address, uint8_t value)
{
  volatile uint32_t* reg = (volatile uint32_t*)(REG_BASE + address * 4);
...
void ESP32can::onInterrupt(void* arg)
{
  ((ESP32can*)arg)->handleInterrupt();
}
