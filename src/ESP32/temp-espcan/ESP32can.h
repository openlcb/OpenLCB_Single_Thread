...
public:
  ESP32can();
   ...
   int txReady();
   int write(uint32_t _txId, uint8_t _txLength, uint8_t* _txData, bool _txExtended);
   int write(uint32_t _txId, uint8_t _txLength, uint8_t* _txData);
   int avail();
   int read(uint32_t &_rxId, uint8_t &_rxDlc, uint8_t* _rxData, bool &_rxExtended);
